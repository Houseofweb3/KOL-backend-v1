...
import {
    getProposalCart,
    getProposalCartBySlugPath,
    submitProposal,
    submitProposalBySlugPath,
} from '../../../services/v1/web/proposal.service';
 ...
 */
/**
 * GET /web/proposal/slug/:clientSlug/:date/:cartId
 * Readable proposal URL; path must match cart (slug, created date UTC, cart id) and link must be unused.
 */
export const getProposalBySlugPathController = async (req: Request, res: Response) => {
    try {
        const { clientSlug, date, cartId } = req.params;
        if (!clientSlug?.trim() || !date?.trim() || !cartId?.trim()) {
            return res.status(HttpStatus.BAD_REQUEST).json({ error: 'Invalid path' });
        }
 ...
 */
export const submitProposalBySlugPathController = async (req: Request, res: Response) => {
    try {
        const { clientSlug, date, cartId } = req.params;
        if (!clientSlug?.trim() || !date?.trim() || !cartId?.trim()) {
            return res.status(HttpStatus.BAD_REQUEST).json({ error: 'Invalid path' });
